...
- **空响应重试**（可选）：当上游返回 HTTP 200 但响应内容为空时，可自动重试到下一个渠道；空响应不触发熔断，仅清除 sticky 偏好。通过 `retry_on_empty_response` 配置项启用（默认关闭）。
...
```jsonc
{
  ...
  // 空响应重试（可选）
  "retry_on_empty_response": false,  // 是否对空响应进行重试，默认关闭
...
- [x] **v0.9.1 (Stability)**:
  - **指数退避熔断**: 连续失败时冷却时间按指数增长，成功后立即重置。
  - **增量同步优化**: 仅同步有变更的服务站，减少不必要的数据库操作。
  - **保底机制**: 所有候选被熔断时，自动选择权重最高渠道进行最后尝试。
- [x] **v0.9.2 (UX & Resilience)**:
  - **空响应重试**: 可选功能，当上游返回 HTTP 200 但内容为空时自动重试到下一渠道。
